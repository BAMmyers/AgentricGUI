...
def call_gemini_assistant(prompt, api_key, node_graph=None):
    ""\"
            ...
            if node:
                try:
                    ...
                    if property_name not in node.properties():
                         response_text = f""Error: Node '{node_name}' does not have property '{property_name}'.""
                    else:
                        ...
                        if isinstance(current_value, bool):
                            if new_value.lower() == 'true':
                                ...
                                response_text = f""Error: Invalid value '{new_value}' for boolean property '{property_name}'.""
                        elif isinstance(current_value, int):
                            try:
                                new_value = int(new_value)
                            except ValueError:
                                response_text =  f""Error: Invalid value '{new_value}' for integer property '{property_name}'.""
                        elif isinstance(current_value, float):
                            try:
                                new_value = float(new_value)
                            except ValueError:
                                response_text =  f""Error: Invalid value '{new_value}' for float property '{property_name}'.""


                        if response_text == "":
                            node.set_property(property_name, new_value)
                            response_text = f""Successfully changed property '{property_name}' of node '{node_name}' to '{new_value}'.""
...
def check_for_updates(installation_id):
    ...
    try:
        ...
        print(f""Update/blacklist check error: {e}"")
        return None, None, False
    except Exception as e:
        print(f""An unexpected error occurred during update/blacklist check: {e}"")
        return None, None, False
